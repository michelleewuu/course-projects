...
public class TestJamboree<M extends Move<M>, B extends Board<M, B>> extends AbstractSearcher<M, B> {
	...
	private class Jamboree extends RecursiveTask<BestMove<M>> {
		...
		protected BestMove<M> compute() {
			...
			if (flag == 1) {
				...
				if (ply <= cutoff) {
					return TestAlphaBeta.alphaBeta(evaluator, board, ply, alpha, beta);
				}

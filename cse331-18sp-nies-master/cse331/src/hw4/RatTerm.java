...
public final class RatTerm {
  ...
  public double eval(double d) {
	if (this.isNaN()) {
		...
		double value = coeff * Math.pow(d,(double)this.expt);
		return value;
	}
  }
  ...
  public RatTerm negate() {
	if (this.isNaN()) {
		...
		RatNum negcoeff = this.coeff.negate();
		return new RatTerm(negcoeff, this.expt);
	}
  }
  ...
  public RatTerm add(RatTerm arg) {
	arg.checkRep();
	if (this.expt != arg.expt && !(arg.isNaN() || arg.isZero() ||
	this.isNaN() || this.isZero())) {
		throw new IllegalArgumentException();
	} else if (this.isNaN() || arg.isNaN()) {
		return RatTerm.NaN;
	} else {
		RatNum coeffsum = this.coeff.add(arg.coeff);
		int exptsum = Math.max(this.expt, arg.expt);
		return new RatTerm(coeffsum, exptsum);
	}
  }
  ...
  public RatTerm mul(RatTerm arg) {
	...
	if (this.isNaN() || arg.isNaN()) {
		...
		RatNum coeffmul = this.coeff.mul(arg.coeff);
		int exptmul = this.expt + arg.expt;
		return new RatTerm(coeffmul, exptmul);
	}
  }
  ...
  public RatTerm div(RatTerm arg) {
	...
	if (arg.isZero() || arg.isNaN() || this.isNaN()) {
		...
		RatNum coeffdiv = this.coeff.div(arg.coeff);
		int exptdiv = this.expt - arg.expt;
		return new RatTerm(coeffdiv, exptdiv);
	}
  }
  ...
  public RatTerm differentiate() {
	if (this.isNaN()) {
		...
		RatNum diffcoeff = this.coeff.mul(new RatNum(this.expt));
		return new RatTerm(diffcoeff, this.expt - 1);	
	}	
  }
  ...
  public RatTerm antiDifferentiate() {
	if (this.isNaN()) {
		...
		RatNum anticoeff = this.coeff.div(new RatNum(this.expt + 1));
		return new RatTerm(anticoeff, this.expt + 1);	
	}
  }
